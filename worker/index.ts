...
async function handleNoticias(req: Request, env: Env, url: URL): Promise<Response> {
  ...
  if (req.method === 'POST') {
    const b = await req.json() as Record<string, unknown>
    if (typeof b.imagen === 'string' && b.imagen.length > 1800000) {
      return err('La imagen es demasiado pesada. Prueba con una foto más liviana.', 400)
    }
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      if (path === '/api/users') return await handleUsers(request, env, url, session)
      if (path === '/api/pedidos') return await handlePedidos(request, env)
      if (path === '/api/mantenimientos') return await handleMantenimientos(request, env)
      if (path === '/api/noticias') return await handleNoticias(request, env, url)
      if (path === '/api/sugerencias') return await handleSugerencias(request, env, url)
      if (path === '/api/notas') return await handleNotas(request, env, url, session)

      return err('Ruta no encontrada', 404)
    } catch (e) {
      // Nunca dejamos que una excepción sin controlar devuelva un error genérico:
      // siempre respondemos con JSON legible para que el frontend pueda mostrarlo.
      const message = e instanceof Error ? e.message : 'Error interno del servidor'
      console.error('Unhandled error:', e)
      return err(`Error del servidor: ${message}`, 500)
    }
  },
}
interface Env {
